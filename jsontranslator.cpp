...
//VotingData JsonTranslator::getVoteData(){
...
bool JsonTranslator::read_file() {
    ...
    for (int list_index = 0; list_index < j_list_of_votelists.size(); ++list_index) {
        ...
        list_of_votelists.push_back(votelist);
    }
    this->vote_data.setListOfVotelists(list_of_votelists);
    ...
    for (int list_index = 0; list_index < j_list_of_courses_and_max_members.size(); ++list_index) {
        ...
        list_of_courses_and_max_members.push_back(course);
    }
    this->vote_data.setListOfCoursesAndMaxMembers(list_of_courses_and_max_members);
    return true;
}
